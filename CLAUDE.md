...
| ファイル | 形式 | 内容 | 次分析への用途 |
|---|---|---|---|
| `data/d18_citation_network.graphml` | GraphML | ノード・エッジ属性付き有向グラフ | **主力入力**（NetworkX / igraph / Gephi） |
| `data/d18_citation_network.gexf` | GEXF | 同上 | Gephi 可視化・探索 |
| `data/d18_edges.csv` | CSV | エッジリスト（weight, year, examiner_cited 等） | pandas 分析・汎用 |
| `data/d18_nodes.csv` | CSV | ノード属性（in/out/degree, PageRank） | pandas 分析・汎用 |
| `output/d18_citation_network.png` | PNG | ネットワーク可視化図 | 確認用 |
| `output/d18_degree_distribution.png` | PNG | 次数分布ヒストグラム | 確認用 |
| `data/triplets_type1.csv` | CSV | トリプレット A→B→C（2,164件） | トリプレット分析入力 |
| `data/triplets_type2.csv` | CSV | トリプレット A→B←C（2,255件） | トリプレット分析入力 |
| `data/triplets_type3.csv` | CSV | トリプレット A←B←C（2,164件） | トリプレット分析入力 |
| `data/triplets_type1_enriched.csv` | CSV | タイプ1 + similarity/judgment/reason（各エッジ） | 次分析入力 |
| `data/triplets_type2_enriched.csv` | CSV | タイプ2 + similarity/judgment/reason（各エッジ） | 次分析入力 |
| `data/triplets_type3_enriched.csv` | CSV | タイプ3 + similarity/judgment/reason（各エッジ） | 次分析入力 |
| `output/fig_triplet1_scatter.png` | PNG | タイプ1 散布図（A→B vs B→C） | 論文図 |
| `output/fig_triplet2_scatter.png` | PNG | タイプ2 散布図（A→B vs C→B） | 論文図 |
| `output/fig_triplet3_scatter.png` | PNG | タイプ3 散布図（B→A vs C→B） | 論文図 |
| `output/fig_triplet_heatmap.png` | PNG | 判定ヒートマップ（3パネル共通カラーバー） | 論文図 |
| `output/fig_sim_cdf.png` | PNG | コサイン類似度 CDF（Similar/Non-similar/All/Random 4線） | 論文図 |
| `output/fig_degree_combined.png` | PNG | 次数分布（Undirected, N=1,530、単一パネル） | 論文図 |
| `output/fig_subgraph_size.png` | PNG | サブグラフサイズ分布（ノード数 vs グラフ数、N=252） | 論文図 |
| `data/network_summary.csv` | CSV | 基本統計・次数・エッジ重み（key-value形式） | 統計参照 |
| `data/network_pagerank.csv` | CSV | PageRank 上位10ノード | 統計参照 |
| `data/network_component_dist.csv` | CSV | サブグラフサイズ分布（ノード数→グラフ数） | 統計参照 |

統計情報: [`docs/network_stats.md`](docs/network_stats.md)
トリプレット分析: [`docs/triplet_analysis.md`](docs/triplet_analysis.md)
コサイン類似度 CDF: [`docs/sim_histogram.md`](docs/sim_histogram.md)
